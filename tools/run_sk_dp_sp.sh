#!/bin/bash
# Targeted screen: StreamK + DataParallel + SplitK only (~147 kernels, ~74 batches)
...
RESULTS_DIR=${RESULTS_DIR:-$WS/results_sk_dp_sp}
log() { echo "[$(date +%H:%M:%S)] $*"; }
...
# Build targeted batch list
grep -lE "DataParallel|StreamK_[0-9]|SplitK_[0-9]" $WS/builds/batch_*.txt 2>/dev/null | sort -u > /tmp/target_batches.txt
TOTAL=$(wc -l < /tmp/target_batches.txt)
log "Processing $TOTAL batches (StreamK+DP+SplitK only)"
...
while read bf; do
  ...
  # Remove .DELETE_ON_ERROR to keep .o when link fails
  ...
  if [ ! -s "$OBJ" ]; then
    log "[$count/$TOTAL] $bid COMPILE FAIL"
